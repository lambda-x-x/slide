...
\begin{block}{相対化された計算可能関数}
入力ストリーム（神託）$f$において，コード$e$神託付き$TM$で計算される関数を$[e]^f$と表す
\end{block}
$\chi_{Halt}^g(<e,n>)=1 \Leftrightarrow [e]^g(n) \downarrow$ である．
また，$Halt^g = \{<e,n> |\ [e]^g(n) \downarrow \}$を表す．
...
\begin{block}{チューリングジャンプによる無限増大列}
